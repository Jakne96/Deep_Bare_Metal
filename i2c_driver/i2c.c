/*
 * i2c.c
 *  i2c driver working on STMF401RE nucleo board
...
void i2c1_init(void)
{
	...
	GPIOB->MODER &= ~(1U<<16); //PB8
	GPIOB->MODER |= 1U<<17;

	GPIOB->MODER &= ~(1U<<18); //PB9
	GPIOB->MODER |= (1U<<19);
...
void i2c1_byte_read(char saddr, char maddr, char* data)
{
    ...
    I2C1->DR = saddr << 1;//Adres jest przesunięty o bit ogólnie dlatego tak
    ...
    I2C1->CR1 &= ~(CR1_ACK);
	/* Clear addr flag */
    tmp = I2C1->SR2;//Czyli to można wyczyścić po prostu odczytując? Śmiesznie
